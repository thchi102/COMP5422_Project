...
def main(args):
    ...
    #Configuring the OpenAI API service    
    ...
    while True:
        ...
        merged_result_url = local_image_to_data_url(merged_result_path)
        print("Waiting for OpenAI API response...")
        answer = prompt_openai(client=gpt_client,model=openai_docs["model"],data_url=merged_result_url,query=user_query,num_keyframes=len(keyframes))
        print(f"GPT-4o thinking: {answer}")
